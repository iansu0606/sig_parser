function [BER,SNR,Pam4levelmeanEQ,Pam4levelstdEQ,RxSymbolEQ]=EQanalysis(inputdata,setting,ref,reflength,TxSymbol,ploten,name)
...
%% Choose Decision Points
    ...
    Tx.SymbolEQ=TxSymbol(reflength+1:end-ceil((ShiftLength2-1)/spb/upsamplenumber));
...
%% BER Count After EQ
    ...
    RxSymbolEQ=Rx.SymbolEQ;
if length(Rx.SymbolEQ)~=length(Tx.SymbolEQ(1+countstart:end-1+countstart))
    length(Rx.SymbolEQ)
    length(Tx.SymbolEQ(1+countstart:end-1+countstart))
else
    BER.countEQ=sum(sum(dec2bin(Rx.SymbolEQ(1:end),PAM_order) ~= dec2bin(Tx.SymbolEQ(1+countstart:end-1+countstart),PAM_order)))/length(dec2bin(Rx.SymbolEQ,PAM_order))/log2(PAM_order);
end
...
if mod(length(Rx.DataEQcorrelate),eyenumber*spb*upsamplenumber)~=0
    ...
    histeye(Rx.outputeye,setting,name1,[10,8,100],[1 0],0,90)
