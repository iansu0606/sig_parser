...
%     Rxpower=load('C:\Users\Jim\Desktop\EQ\data\50G_2km\Rx_10.txt');
    Txpower=load('C:\Users\Jim\Desktop\EQ\data\100G_500m\Tx_10.txt');
    Rxpower=load('C:\Users\Jim\Desktop\EQ\data\100G_500m\Rx_10.txt');
    Pam4Level1 = 0.025;
    Pam4Level2 = 0.055;
    Pam4Level3 = 0.09;
    Pam4Level4 = 0.12;
...
eyeplot(Rx.inputeye.',sample_rate,spb,'Eyediagram of Data after filter',PAM_order)
histeye(Rx.inputeye,sample_rate,spb,'Eyediagram of Data after filter',PAM_order)
...
Databitupsample=resample(Rx.Databitnormalize,sample_rate,symbol_rate);
Databitupsamplere=reshape(Databitupsample(1:2*spb*floor(length(Tx.Refcorrelate)/2/spb)),2*spb,[]);
% eyeplot(Databitupsamplere,sample_rate,spb,'Eyediagram of Data before EQ',PAM_order);
% histeye(Databitupsamplere,sample_rate,spb,'Eyediagram of Data before EQ',PAM_order);
...
elseif PAM_order==4
%     lmssize=0.005;
%     lmssize2=lmssize/1e1;    
    lmssize=0.001;
    lmssize2=lmssize/1e1;
end

[Rx.DataFFE data_rx_eq errCount]=ffeblock(Rx.Databitnormalize,Ref,PAM_order,lmssize,fftap,0);
% Rx.DataFFE=Rx.DataFFE/mean(abs(Rx.DataFFE))*log2(PAM_order);
DFERef=data_rx_eq;
Rx.DataDFE=dfeblock2(Rx.Databitnormalize,DFERef,PAM_order,lmssize,lmssize2,fftap,dfetap);
...
Rx.DataEQ=resample(Rx.DataEQ,sample_rate,symbol_rate).';
...
Tx.EQ=Tx.normalized(2:(2^PRBS_length-2-floor(ShiftLength2/spb))*spb,1);

Tx.TrainEQ=Tx.Train(2:(2^PRBS_length-2-floor(ShiftLength2/spb)));
Tx.SymbolEQ=Tx.Symbol(2:(2^PRBS_length-2-floor(ShiftLength2/spb)));
...
%% Eye Diagram
    ...
    Rx.outputeye = reshape(Rx.DataEQcorrelate(length(Ref)*spb+1:2*spb*floor(length(Tx.EQ)/2/spb)),2*spb,[]);
    Rx.DataEQreshape = reshape(Rx.DataEQcorrelate,spb,[]);    
...
%% BER Count After EQ
    ...
    BER.countEQ=sum(sum(dec2bin(Rx.SymbolEQ(1:end-1)) ~= dec2bin(Tx.SymbolEQ(reflength+1:end))))/length(dec2bin(Rx.SymbolEQ))/log2(PAM_order);
